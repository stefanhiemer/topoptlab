...
def demonstrate_alm_line(nvars: int = 3,
                         ...
                         maxiter: int = 2000,
                         maxinnerit: int = 5,
                         start_constrained: bool = False,
                         rho: float = 1e0,
                         rho_scale: int = 1.05,
                         rho_max: float = 1e2,
                         eps_eq: float = 1e-8,
                         use_viability_policy: bool = True,
                         feas_tol: float = 1e-5) -> Tuple[np.ndarray,
                                                        np.ndarray, 
                                                        np.ndarray]:
    """
    ...
    for i in range(maxiter):
        ...
        for j in range(maxinnerit):
            ...
            if use_viability_policy and np.abs(ceq).max() > feas_tol:
                fgrad_step = np.zeros_like(fgrad)
            else:
                fgrad_step = fgrad
        ...
        if rho < rho_max and \
           (np.abs(ceq) > eps_eq) and\
           (np.abs(ceq) > np.abs(ceqold)):
            rho = rho * rho_scale
        #
