...
@pytest.mark.parametrize('Es, nus, _as, xe',
                         [([1.],[0.3],[0.05], 
...
@pytest.mark.parametrize('cs, _as, xe',
...
def test_anisotrop_linelast_2d(cs,_as, xe):
    ...
    #
    if len(cs.shape) == 2:
        Kes = fk_heatexp_aniso_2d(c=cs,a=_as)[None,:,:]
    else:
        Kes = stack([fk_heatexp_aniso_2d(c=cs[i],a=_as[i]) for i in range(cs.shape[0])])
    #
    assert_almost_equal(_fk_heatexp_2d(xe=xe,c=cs,a=_as),
                        Kes)
    return
...
def test_anisotrop_linelast_3d(cs,_as, xe):
    ...
    if len(cs.shape) == 2 and xe.shape[0]==1:
        Kes = fk_heatexp_aniso_3d(c=cs,a=_as)[None,:,:]
    elif len(cs.shape) == 2 and xe.shape[0]!=1:
        Kes = stack([fk_heatexp_aniso_3d(c=cs,a=_as) for i in range(xe.shape[0])])
    else:
        Kes = stack([fk_heatexp_aniso_3d(c=cs[i],a=_as[i]) for i in range(cs.shape[0])])
