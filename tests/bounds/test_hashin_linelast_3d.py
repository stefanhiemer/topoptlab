...
from topoptlab.bounds.hashin_shtrikman_3d import emod_binary_low,emod_nary_low

@pytest.mark.parametrize('npoints, Kmin, Kmax, Gmin, Gmax',
                         [(11,1e-2,1.,2e-2,1.),
                          (11,0.5,1.,0.4,1.),])

def test_binary_youngs_low(npoints,Kmin,Kmax,Gmin,Gmax):
    
    x = linspace(0,1,npoints)
    
    assert_almost_equal(emod_binary_low(x, 
                                        Kmin = Kmin, Kmax = Kmax,
                                        Gmin = Gmin, Gmax = Gmax), 
                        emod_nary_low(x[:,None],
                                      Ks = array([Kmax,Kmin]),
                                      Gs = array([Gmax,Gmin])))
    assert_almost_equal(flip(emod_binary_low(x, 
                                             Kmin = Kmin, Kmax = Kmax,
                                             Gmin = Gmin, Gmax = Gmax)), 
                        emod_nary_low(x[:,None], 
                                      Ks = array([Kmin,Kmax]),
...
from topoptlab.bounds.hashin_shtrikman_3d import poiss_binary_low,poiss_nary_low

@pytest.mark.parametrize('npoints, Kmin, Kmax, Gmin, Gmax',
                         [(11,1e-2,1.,2e-2,1.),
                          (11,0.5,1.,0.4,1.),])

def test_binary_poiss_low(npoints,Kmin,Kmax,Gmin,Gmax):
    
    x = linspace(0,1,npoints)
    
    assert_almost_equal(poiss_binary_low(x, 
                                         Kmin = Kmin, Kmax = Kmax,
                                         Gmin = Gmin, Gmax = Gmax), 
                        poiss_nary_low(x[:,None],
                                       Ks = array([Kmax,Kmin]),
                                       Gs = array([Gmax,Gmin])))
    assert_almost_equal(flip(poiss_binary_low(x, 
                                              Kmin = Kmin, Kmax = Kmax,
                                              Gmin = Gmin, Gmax = Gmax)), 
                        poiss_nary_low(x[:,None], 
                                       Ks = array([Kmin,Kmax]),
...
from topoptlab.bounds.hashin_shtrikman_3d import emod_binary_upp,emod_nary_upp

@pytest.mark.parametrize('npoints, Kmin, Kmax, Gmin, Gmax',
                         [(11,1e-2,1.,2e-2,1.),
                          (11,0.5,1.,0.4,1.),])

def test_binary_youngs_upp(npoints,Kmin,Kmax,Gmin,Gmax):
    
    x = linspace(0,1,npoints)
    
    assert_almost_equal(emod_binary_upp(x, 
                                        Kmin = Kmin, Kmax = Kmax,
                                        Gmin = Gmin, Gmax = Gmax), 
                        emod_nary_upp(x[:,None],
                                      Ks = array([Kmax,Kmin]),
                                      Gs = array([Gmax,Gmin])))
    assert_almost_equal(flip(emod_binary_upp(x, 
                                             Kmin = Kmin, Kmax = Kmax,
                                             Gmin = Gmin, Gmax = Gmax)), 
                        emod_nary_upp(x[:,None], 
                                      Ks = array([Kmin,Kmax]),
...
from topoptlab.bounds.hashin_shtrikman_3d import poiss_binary_upp,poiss_nary_upp

@pytest.mark.parametrize('npoints, Kmin, Kmax, Gmin, Gmax',
                         [(11,1e-2,1.,2e-2,1.),
                          (11,0.5,1.,0.4,1.),])

def test_binary_poiss_upp(npoints,Kmin,Kmax,Gmin,Gmax):
    
    x = linspace(0,1,npoints)
    
    assert_almost_equal(poiss_binary_upp(x, 
                                         Kmin = Kmin, Kmax = Kmax,
                                         Gmin = Gmin, Gmax = Gmax), 
                        poiss_nary_upp(x[:,None],
                                       Ks = array([Kmax,Kmin]),
                                       Gs = array([Gmax,Gmin])))
    assert_almost_equal(flip(poiss_binary_upp(x, 
                                              Kmin = Kmin, Kmax = Kmax,
                                              Gmin = Gmin, Gmax = Gmax)), 
                        poiss_nary_upp(x[:,None], 
                                       Ks = array([Kmin,Kmax]),
...
from topoptlab.bounds.hashin_shtrikman_3d import emod_binary_low_dx

@pytest.mark.parametrize('npoints, Kmin, Kmax, Gmin, Gmax',
                         [(11,1e-2,1.,2e-2,1.),
                          (11,0.5,1.,0.4,1.),])

def test_binary_youngs_low_dx(npoints,Kmin,Kmax,Gmin,Gmax):
    
    x = linspace(0,1,npoints)
    
    assert_almost_equal(emod_binary_low_dx(x, 
                                           Kmin = Kmin, Kmax = Kmax,
                                           Gmin = Gmin, Gmax = Gmax), 
                        derivative(emod_binary_low,x,args=(Kmin,Kmax,
                                                           Gmin,Gmax)).df)
    return

from topoptlab.bounds.hashin_shtrikman_3d import poiss_binary_low_dx

@pytest.mark.parametrize('npoints, Kmin, Kmax, Gmin, Gmax',
                         [(11,1e-2,1.,2e-2,1.),
                          (11,0.5,1.,0.4,1.),])

def test_binary_poiss_low_dx(npoints,Kmin,Kmax,Gmin,Gmax):
    
    x = linspace(0,1,npoints)
    
    assert_almost_equal(poiss_binary_low_dx(x, 
                                            Kmin = Kmin, Kmax = Kmax,
                                            Gmin = Gmin, Gmax = Gmax), 
                        derivative(poiss_binary_low,x,args=(Kmin,Kmax,
                                                            Gmin,Gmax)).df)
    return
...
from topoptlab.bounds.hashin_shtrikman_3d import emod_binary_upp_dx

@pytest.mark.parametrize('npoints, Kmin, Kmax, Gmin, Gmax',
                         [(11,1e-2,1.,2e-2,1.),
                          (11,0.5,1.,0.4,1.),])

def test_binary_youngs_upp_dx(npoints,Kmin,Kmax,Gmin,Gmax):
    
    x = linspace(0,1,npoints)
    
    assert_almost_equal(emod_binary_upp_dx(x, 
                                           Kmin = Kmin, Kmax = Kmax,
                                           Gmin = Gmin, Gmax = Gmax), 
                        derivative(emod_binary_upp,x,args=(Kmin,Kmax,
                                                           Gmin,Gmax)).df)
    return

from topoptlab.bounds.hashin_shtrikman_3d import poiss_binary_upp_dx

@pytest.mark.parametrize('npoints, Kmin, Kmax, Gmin, Gmax',
                         [(11,1e-2,1.,2e-2,1.),
                          (11,0.5,1.,0.4,1.),])

def test_binary_poiss_upp_dx(npoints,Kmin,Kmax,Gmin,Gmax):
    
    x = linspace(0,1,npoints)
    
    assert_almost_equal(poiss_binary_upp_dx(x, 
                                            Kmin = Kmin, Kmax = Kmax,
                                            Gmin = Gmin, Gmax = Gmax), 
                        derivative(poiss_binary_upp,x,args=(Kmin,Kmax,
                                                            Gmin,Gmax)).df)
    return
...
from topoptlab.bounds.hashin_shtrikman_3d import emod_nary_low_dx

@pytest.mark.parametrize('npoints, Ks, Gs',
                         [(11,array([1e-2,1.]),array([2e-2,1.])),
                          (11,array([0.5,1.]),array([0.4,1.])),])

def test_nary_youngs_low_dx(npoints,Ks,Gs):
    ...
    for i in range(x.shape[1]):
        dx = zeros(x.shape)
        dx[:,i] += d
        jac[:,i] = (emod_nary_low(x+dx,Ks = Ks,Gs = Gs) - y0)/d
    
    assert_almost_equal(emod_nary_low_dx(x, 
                                         Ks = Ks,
                                         Gs = Gs),
                        jac,
                        decimal=5)
...
from topoptlab.bounds.hashin_shtrikman_3d import emod_nary_upp_dx

@pytest.mark.parametrize('npoints, Ks, Gs',
                         [(11,array([1e-2,1.]),array([2e-2,1.])),
                          (11,array([0.5,1.]),array([0.4,1.])),])

def test_nary_youngs_upp_dx(npoints,Ks,Gs):
    ...
    for i in range(x.shape[1]):
        dx = zeros(x.shape)
        dx[:,i] += d
        jac[:,i] = (emod_nary_upp(x+dx,
                                  Ks = Ks,
                                  Gs = Gs) - y0)/d
    
    assert_almost_equal(emod_nary_upp_dx(x, 
                                         Ks = Ks,
                                         Gs = Gs),
                        jac,
                        decimal=5)
...
from topoptlab.bounds.hashin_shtrikman_3d import poiss_nary_low_dx

@pytest.mark.parametrize('npoints, Ks, Gs',
                         [(11,array([1e-2,1.]),array([2e-2,1.])),
                          (11,array([0.5,1.]),array([0.4,1.])),])

def test_nary_poiss_low_dx(npoints,Ks,Gs):
    ...
    for i in range(x.shape[1]):
        dx = zeros(x.shape)
        dx[:,i] += d
        jac[:,i] = (poiss_nary_low(x+dx,Ks = Ks,Gs = Gs) - v0)/d
    
    assert_almost_equal(poiss_nary_low_dx(x, 
                                          Ks = Ks,
                                          Gs = Gs),
                        jac,
                        decimal=5)
...
from topoptlab.bounds.hashin_shtrikman_3d import poiss_nary_upp_dx

@pytest.mark.parametrize('npoints, Ks, Gs',
                         [(11,array([1e-2,1.]),array([2e-2,1.])),
                          (11,array([0.5,1.]),array([0.4,1.])),])

def test_nary_poiss_upp_dx(npoints,Ks,Gs):
    ...
    for i in range(x.shape[1]):
        dx = zeros(x.shape)
        dx[:,i] += d
        jac[:,i] = (poiss_nary_upp(x+dx,
                                   Ks = Ks,
                                   Gs = Gs) - v0)/d
    
    assert_almost_equal(poiss_nary_upp_dx(x, 
                                          Ks = Ks,
                                          Gs = Gs),
                        jac,
                        decimal=5)
