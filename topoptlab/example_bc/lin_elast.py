...
def gripper_2d(nelx: int, nely: int,
               ndof: int, 
               ...
               **kwargs: Any) -> Tuple[np.ndarray,np.ndarray,
                                       ...
                                       np.ndarray]:
    """
    Gripper as example case for compliant mechanisms. We assume a length 
    ...
    #
    fixed = np.union1d(np.arange(1,(nelx+1)*(nely+1)*2,(nely+1)*2), # symmetry
                       np.array([0])) # bottom left fixation
    # load/source
    clamp = np.arange((nely+1)*2-1,
                      ndof,(nely+1)*2)[:int(nelx*input_frac)+1]
    f[clamp,0] = 1
    #
    spring_inds = int(slit_height*nely*2) + np.arange(0,ndof,2*(nely+1)) + 1
    spring_inds = spring_inds[-int(output_frac*nelx):]
    springs = [np.sort(np.hstack((spring_inds, clamp))),
...
def heatexpbench_2d(nelx: int, nely: int,
           ndof: int, **kwargs: Any
           ) -> Tuple[np.ndarray,np.ndarray,np.ndarray,np.ndarray,None]:
    """
    This is a benchmark case similar to 
    
    Jonsmann, Jacques, Ole Sigmund, and Siebe Bouwstra. 
    "Compliant thermal microactuators." Sensors and Actuators A: Physical 76.1-3 (1999): 463-469.
    .
    ...
    free : np.ndarray
        indices of free dofs (ndofs - nfixed).
    springs : list
        contains spring at top left applied to y dof.
    ...
    u = np.zeros((ndof, 1))
    # symmetry bc (fix x displacements to zero)
    fixed = np.hstack((np.arange(0,2*(nely+1),2), # mirror symmetry 
                       np.arange(4*(nely+1)-2, # fix x bottom
                                 2*(nelx+1)*(nely+1),
                                 2*(nely+1)), 
                       np.arange(4*(nely+1)-1, # fix y bottom
                                 2*(nelx+1)*(nely+1),
                                 2*(nely+1)),  
                       np.arange(2*nelx*(nely+1),
                                 ndof-2))) # fix right
    # spring at left top
    springs = [np.array([1]),np.array([0.1])]
    return u,f,fixed,np.setdiff1d(dofs,fixed),springs