...
def main(nelx, nely, volfrac, penal, rmin, ft, 
    ...
    if not pde and ft in [0,1]:
        ...
        iH = iH.at[cc].set(el) # row
        jH = jH.at[cc].set(k*nely+l) #column
        sH = sH.at[cc].set(np.maximum(0.0, fac))
        # Finalize assembly and convert to csc format
        H = BCOO((sH, np.column_stack((iH, jH)).astype(np.int64)), 
    ...
    for loop in np.arange(2000):
        ...
        K = BCOO((sK, np.column_stack((iK, jK)).astype(np.int64)), 
                 shape=(ndof_free, ndof_free))
        # Remove constrained dofs from matrix
        #K = K[free, :][:, free]
        # Solve system(s)
        if u.shape[1] == 1:
            u = u.at[free, 0].set(spsolve(K, f[free, 0]))
        else:
            u = u.at[free,:].set(spsolve(K, f[free, :]))
...
def update_indices(indices,fixed,mask):
    ...
    val = val.at[_mask].set(np.arange(_mask.sum()))
