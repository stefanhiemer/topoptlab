...
from topoptlab.example_bc.lin_elast import threepointbending_2d,xcenteredbeam_2d,selffolding_2d
...
def fem_heat_expansion(nelx, nely, nelz=None,
                       ...
                       kmax=1.0, kmin=1e-9,
                       a1=1e-1,a2=5e-2,Eratio = 0.35,
                       lin_solver="cvxopt-cholmod", preconditioner=None,
                       assembly_mode="full",
                       bcs=[selffolding_2d, # threepointbending_2d
                            rectangle_2d],
    ...
    a = [np.arange(1,ndim+1)*a1 for i in np.arange(int(nely/2))]
    a += [np.arange(ndim,0,-1)*a2 for i in np.arange(int(nely/2),nely)]
    a = np.tile(np.stack(a),(nelx,1,1))
    import sys 
    sys.exit()
    # anisotropic heat conductivity
    ks = [np.eye(ndim)*kmax for i in np.arange(int(nely/2))]
    ks += [np.eye(ndim)*kmax for i in np.arange(int(nely/2),nely)]
