...
def fem_heat_expansion(nelx, nely, nelz=None,
    ...
    a = [np.arange(1,ndim+1)*a2 for i in np.arange(int(nely/2))]
    a += [np.arange(ndim,0,-1)*a1 for i in np.arange(int(nely/2),nely)]
    a = np.tile(np.stack(a),(nelx,1,1))
    # anisotropic heat conductivity
    ks = [np.eye(ndim)*kratio for i in np.arange(int(nely/2))]
    ks += [np.eye(ndim) for i in np.arange(int(nely/2),nely)]
